...
function DLFile($FILE) {
	...
	curl_setopt($ch, CURLOPT_URL, 'http://2moons.googlecode.com/svn/trunk/'.$Path);
...
function getDatafromServer($action)
{
	...
	curl_setopt($ch, CURLOPT_USERAGENT, "2Moons Update API");
	curl_setopt($ch, CURLOPT_CRLF, true);
	curl_setopt($ch, CURLOPT_POSTFIELDS, sprintf(PHPSVN_LOGS_REQUEST, 1638, 1639));
	curl_exec($ch);
	curl_close($ch);
	fclose($fp);	
...
function ShowUpdatePage()
{
	...
	switch($_REQUEST['action'])
	{
		...
		break;
		case "update":
			ExecuteUpdates($Patchlevel);
		break;
		default:
			DisplayUpdates($Patchlevel);
		break;
	}
}
...
function ExecuteUpdates($Patchlevel) {
	...
	foreach($UpdateArray['revs'] as $Rev => $RevInfo) 
	{
		if($Check === 0) {
			$Check = 1;
			if($Rev - 1 != $Level) 
				exitupdate(array('debug' => array('noupdate' => 'UpdateServer require Revision '.($Rev - 1).'.')));
		}
...
function DisplayUpdates($Patchlevel) {
	global $LNG;
	$template 	= new template();
