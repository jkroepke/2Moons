...
class ShowFleetTablePage extends AbstractPage
{
	...
	public function show()
	{
		...
		if(!empty($elementID) && !IsVacationMode($USER))
		{
			switch($GetAction){
				case "sendfleetback":
					FleetUtil::SendFleetBack($USER, $elementID);
				break;
				case "acs":
		...
		$techExpedition      = $USER[$GLOBALS['VARS']['ELEMENT'][124]['name']];
		...
		while ($fleetsRow = $GLOBALS['DATABASE']->fetchArray($fleetResult))
		{
			...
			$FlyingFleetList[]	= array(
				...
				'startTime'		=> DateUtil::formatDate($LNG['php_tdformat'], $fleetsRow['fleet_start_time'], $USER['timezone']),
				'endGalaxy'		=> $fleetsRow['fleet_end_galaxy'],
				'endSystem'		=> $fleetsRow['fleet_end_system'],
				'endPlanet'		=> $fleetsRow['fleet_end_planet'],
				'endTime'		=> DateUtil::formatDate($LNG['php_tdformat'], $fleetsRow['fleet_end_time'], $USER['timezone']),
		...
		$this->assign(array(
			...
			'bonusAttack'			=> $USER[$GLOBALS['VARS']['ELEMENT'][109]['name']] * 10 + (1 + $USER['factor']['Attack']) * 100,
			'bonusDefensive'		=> $USER[$GLOBALS['VARS']['ELEMENT'][110]['name']] * 10 + (1 + $USER['factor']['Defensive']) * 100,
			'bonusShield'			=> $USER[$GLOBALS['VARS']['ELEMENT'][111]['name']] * 10 + (1 + $USER['factor']['Shield']) * 100,
			'bonusCombustion'		=> $USER[$GLOBALS['VARS']['ELEMENT'][115]['name']] * 10 + (1 + $USER['factor']['FlyTime']) * 100 - 100,
			'bonusImpulse'			=> $USER[$GLOBALS['VARS']['ELEMENT'][117]['name']] * 20 + (1 + $USER['factor']['FlyTime']) * 100 - 100,
