...
class ShowFleetAjaxPage
{
	public $returnData	= array();

	private function sendJSON($Code, $Message) {
	...
	public function show()
	{
		...
		$planetID 		= request_var('planetID', 0);
		...
		if (IsVacationMode($USER)) {
			$this->sendJSON(620, $LNG['fa_vacation_mode_current']);
		}
		
		if (empty($planetID)) {
			$this->sendJSON(601, $LNG['fa_planet_not_exist']);
		}
		
		if ($maxSlots <= $activeSlots) {
			$this->sendJSON(612, $LNG['fa_no_more_slots']);
		}
		
		switch($targetMission)
		{
			case 6:
				if(!isModulAvalible(MODUL_MISSION_SPY)) {
					$this->sendJSON(699, $LNG['sys_module_inactive']);
				}
				...
				if(empty($SpyProbes)) {
					$this->sendJSON(611, $LNG['fa_no_spios']);
				}
				$fleetArray = array(210 => $SpyProbes);
			break;
			case 8:
				if(!isModulAvalible(MODUL_MISSION_RECYCLE)) {
					$this->sendJSON(699, $LNG['sys_module_inactive']);
				}
				...
				if(empty($Recycles) && empty($GRecycles)) {
					$this->sendJSON(611, $LNG['fa_no_recyclers']);
				}
					
				$fleetArray = array(209 => $Recycles, 219 => $GRecycles);
				break;
			default:
				$this->sendJSON(610, $LNG['fa_not_enough_probes']);
			break;
		}
		...
		$targetData	= $db->uniquequery("SELECT planet.id_owner as id_owner, 
		...
		if($targetMission == 6)
		{
			if($CONF['adm_attack'] == 1 && $TargetUser['authattack'] > $USER['authlevel']) {
				$this->sendJSON(619, $LNG['fa_action_not_allowed']);
			}
			
			if (IsVacationMode($targetData)) {
				$this->sendJSON(605, $LNG['fa_vacation_mode']);
			}
			
			$IsNoobProtec	= CheckNoobProtec($USER, $targetData, $targetData);
			
			if ($IsNoobProtec['NoobPlayer']) {
				$this->sendJSON(603, $LNG['fa_week_player']);
			}
			
			if ($IsNoobProtec['StrongPlayer']) {
				$this->sendJSON(604, $LNG['fa_strong_player']);
			}

			if ($USER['id'] == $targetData['id_owner']) {
				$this->sendJSON(618, $LNG['fa_not_spy_yourself']);
			}
		}
		...
		if($UserDeuterium < 0) {
			$this->sendJSON(613, $LNG['fa_not_enough_fuel']);
		}
		
		if($consumption > FleetFunctions::GetFleetRoom($fleetArray)) {
			$this->sendJSON(613, $LNG['fa_no_fleetroom']);
		}
		...
		$this->sendJSON(600, $LNG['fa_sending']." ".array_sum($fleetArray)." ". $LNG['tech'][$shipID[0]] ." ".$LNG['gl_to']." ".$targetData['galaxy'].":".$targetData['system'].":".$targetData['planet']." ...");
	}
}
?>