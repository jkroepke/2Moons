...
function printArray($Array, $Tab) {
	...
	foreach($Array as $Key => $Value) {
		echo str_repeat(TAB, $Tab),(is_numeric($Key) ? $Key : "'".$Key."'"),TAB,str_repeat(TAB, $MaxKey - ceil((strlen($Key) + (is_numeric($Key) ? 0 : 2)) / 4)),'=> ';
		if(is_array($Value)) {
			echo printArray($Value, $Tab);
	...
	$Tab	-= 1;
	echo str_repeat(TAB, $Tab).')';
}
$i	= 0;
echo '<?php',NL,'$ELEMENT	= ',printArray($ELEMENT, $i),';';
