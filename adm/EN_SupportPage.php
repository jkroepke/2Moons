...
/// Deteilsanzeige des eigenen tickets
		...
		while($ticket = $db->fetch_array($query)){
			switch($ticket['status']){
				...
				break;
				case 2:
				$status = "<font color=orange>Admin answer</font>";
				break;
				case 3:
...
/// Pr¸fen ob beide felder mit Text versehen sind
		...
		while($ticket = $db->fetch_array($query))
		{
		$newtext = $ticket['text'].'<br><br><hr>'.$user['username'].'(Admin) wrote on '.date("j. M Y H:i:s", time()).'<br><br><font color="red">'.$antworttext.'</font>';
		...
		SendSimpleMessage($SuppTicket['player_id'], '', time(), 4, $user['username'], "Support Ticket #".$antwortticketid, "On your ticket #".$antwortticketid." a reply was posted!");
		...
		$newtext = $ticket['text'].'<br><br><hr>'.$user['username'].'(Admin) closed the ticket on '.date("j. M Y H:i:s", time()).' !';
		$QryUpdatemsg  = "UPDATE ".SUPP." SET ";
		$QryUpdatemsg .= "`text` = '".$db->sql_escape($newtext)."',";
