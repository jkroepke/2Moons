	...
	for ($ROUNDC = 0; $ROUNDC < MAX_ATTACK_ROUNDS; $ROUNDC++) 
	{
		...
		foreach ($attackers as $fleetID => $attacker) {
			...
			foreach($attacker['detail'] as $element => $amount) {
				...
				$max_removePoints = max($max_removePoints, 0);
				$ile_removePoints = min(max($ile_removePoints, 0), $ile_removePoints);
			...
			foreach ($defenders as $fleetID2 => $defender) {
				foreach($attacker['detail'] as $element => $amount) {
					...
					foreach ($CombatCaps[$element]['sd'] as $c => $d) {
						...
						$defender_n[$fleetID2][$c] = max($defender_n[$fleetID2][$c], 0);
					}
				}
				...
				foreach($defender['def'] as $element => $amount) {
					...
					foreach ($CombatCaps[$element]['sd'] as $c => $d) {
						...
						$attacker_n[$fleetID][$c] = max($attacker_n[$fleetID][$c], 0);
					}
				}
			}
		}
