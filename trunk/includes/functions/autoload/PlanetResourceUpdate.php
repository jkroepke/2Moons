...
function PlanetResourceUpdate ( $CurrentUser, &$CurrentPlanet, $UpdateTime, $Simul = false )
{
	...
	else
	{
		...
		$Caps['metal_perhour'] = $Caps['crystal_perhour'] = $Caps['deuterium_perhour'] = $Caps['energy_used'] = $Caps['energy_max'] = 0;
		...
		foreach($reslist['prod'] as $id => $ProdID)
		{
			...
			$Caps['deuterium_perhour'] += floor(eval($ProdGrid[$ProdID]['formule']['deuterium']) * (0.01 * $post_porcent) * ($game_config['resource_multiplier']).(($ProdID == 12) ? ( * 1 + ( $CurrentUser['rpg_geologue'] * GEOLOGUE )) : 1);
