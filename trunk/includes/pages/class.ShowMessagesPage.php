...
{	
	...
	function DelMessages()
	{
		...
		switch($DeleteWhat)
		{
			...
			break;
			case 'deletetypeall':
				$db->multi_query("DELETE FROM ".MESSAGES." WHERE `message_owner` = '".$_SESSION['id']."' AND `message_type` = '".$MessCategory."';UPDATE ".USERS." SET `new_message` = '0', `new_gmessage` = '0' WHERE `id` = '".$_SESSION['id']."';");
			case 'deletemarked':
				$SQLWhere = array();
