	...
	if (!defined('LOGIN') && !defined('IN_CRON'))
	{
		...
		if (isset($user))
		{
			...
			{	
				...
				$fleetquery = $db->query("SELECT * FROM ".FLEETS." WHERE (`fleet_start_time` <= '". time() ."' AND `fleet_mess` = '0') OR (`fleet_end_time` <= '". time() ."' AND `fleet_mess` = '1') ORDER BY `fleet_start_time` ASC;");
