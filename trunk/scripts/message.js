	...
	GenerateMessages: function () {
		...
		$.each(data, function(tmp, mess) {
			if((mess.type == Message.MessID || Message.MessID == 100 || Message.MessID == 999) && !$.isEmptyObject(mess)) {
				...
				TEMP += (mess.type == 1 && Message.MessID != 999) ? '<a href="#" onclick="return Dialog.PM('+mess.sender+', \''+Message.CreateAnswer(mess.subject)+'\', \''+encodeURIComponent(mess.text)+'\');" title="'+Message.LNG.mg_answer_to+' '+Message.stripHTML(mess.from)+'"><img src="'+Skin+'img/m.gif" border="0"></a>' : '';
