...
$LNG['rules']                          = array(
      "Contas"                      => "    
    * Cada jogador apenas pode possuir uma conta por universo, mais do que uma será considerada multi-conta e as contas envolvidas serão sancionadas com o bloqueamento, e considerado uma infração muito grave.
    ...
    * Se duas ou mais contas partilharem regularmente o mesmo IP, nenhuma dessas contas poderá ser MoonMaker ( faz luas) ou Moon Destroyers( destroidor de luas).",

      "Multi-Contas"                  => "    
    * Cada jogador apenas pode possuir uma conta por universo, mais do que uma será considerada multi-conta e as contas envolvidas serão sancionadas com o bloqueamento temporário ou permanete.
    * Quando o jogador normal ou ocasionalmente jogar a partir de um IP público (cyber-café, universidades, escolas, etc), um Admin de jogo do universo em causa deve ser obrigatoriamente notificado.
    * Se contas de proprietários diferentes são normal, ocasional ou permanentemente jogadas a partir do mesmo IP, um Admin de jogo do universo em causa deve ser obrigatoriamente notificado.
    * Nestes casos, as contas não estão autorizadas em nenhum caso a terem interacção de frotas que inclui ataques, trocas, envios de recursos, envio de sondas, etc.
    * Perfis que partilhem um mesmo IP, seja em casa ou no trabalho, também não têm permissão de fora desse IP, fazerem interacções de frota incluindo ataques em ACS ou empréstimo de recicladores, a única excepção é se essa partilha de IP resultar num sitting de uma das contas - neste caso apenas poderão fazer interacção fora de um mesmo IP.
   ...
   * É considerado pushing:
    ...
    * São excepções (em alguns casos devidamente autorizados por um Administrador):
        ...
        "Bashing"                          => " 
    * Mais de 6 ataques em 24 horas, ao mesmo planeta de um jogador activo, é considerado Bashing, e por isso não é permitido. Em caso de denucio deverá ser entregue provas de concretas de tais acontecimentos.
    ...
    * Devido ao risco de Destruição de Lua este tipo de missão não é considerado para a regra do bashing.
    ...
    * As declarações de guerra entre alianças não alteram o cumprimento desta regra.",
        ...
        "IP`s"                   => "
    * A verificação de IP de uma conta só pode ser desactivada mediante autorização de um admin do jogo através de ticket.
    ...
    * A Equipa jogo não irá responsabilizar-se por perdas de contas com a Verificação de IP desactivada. ",
       
        "Bugusing"                        => "
    * Não é permitido ser favorecido através do uso de bugs e/ou erros de programação e ainda de scripts.
    * Se um bug for identificado por um usuário, este deverá reportá-lo o mais rapidamente possível, através do suporte do jogo ou no fórum de Bugs, ou por email.
    * Quaisquer acções que interfiram no bom funcionamento do jogo são proibidas.
    * É proibido manter a verificação de IP desligada se esta não tiver sido autorizada por o Adminstrador de Jogo.",

        "Línguagem e Atitudes"                => "
    * A língua oficial dos uníversos jogo é o português. No entanto, qualquer pessoa, de qualquer nacionalidade, pode jogar no jogo, desde que não use outra língua a não ser a oficial no jogo, quer através de mensagens internas com outros jogadores ou informação nas páginas de alianças. 
    * Da mesma maneira, spam, nomes ou mensagens no jogo de conteúdo obsceno, insultuoso, racista, xenófobo ou terrorista conduzem à exclusão do jogo. ",
     ...
     * Desrespeito, insultos, racismo, xenofobia, ameaças à vida real, difamação ou qualquer tipo de perseguição feita a elementos da equipa do jogo, leva à exclusão do jogador infractor da comunidade do jogo, ou seja, ao bloqueamento permanente em todos os universos que joga.",
     ...
     * Também o envio de mensagens com carácter publicitário de outros jogos ou sites de Internet que não sejam relacionados com o jogo em si, poderam ser alvo de aviso ou em caso extremos de bloqueamento.
     ...
     * Queixas relativas a estas sanções não serão tidas em conta e a palavra final será sempre da equipa do jogo, a resposta no mesmo tom  pode implicar um Ban nos 2 intervenientes.",

        "Regras"                      => " 
     * As regras podem mudar, e cada usuário é obrigado a constantemente verificar se ouve alteração neste mesmo sítio!",
...
$LNG['agb']                            = array(
        ...
        "Objecto das condições de utilização; área de aplicação"             => array(
                ...
                "Os jogos online e os outros serviços disponibilizados pelo  jogo serve apenas de entretenimento. É proibida a utilização para efeitos de aquisição ou efeitos comerciais.",
                
                "O utilizador é responsável pela actualização e adequação do software e hardware que utiliza.",
                
                "Adicionalmente a estas condições de utilização, são válidas as regras do jogo válidas para o respectivo jogo online, caso existam. No caso de contradições entre estas condições de utilização e as regras do jogo, prevalecem as disposições destas condições de utilização. Para jogos online individuais, variantes especiais e/ou partes integrantes dos jogos online e ofertas de serviços individuais nas páginas de Internet deste site são válidas, para além disso, condições de utilização especiais. Antes da utilização da respectiva oferta, o utilizador é chamado a atenção para isso de uma forma adequada.",
                
                "O utilizador tem a possibilidade de utilizar serviços de parceiros contratuais do jogo online individual e outros serviços. Neste caso, celebra-se um contrato em separado entre o utilizador e o respectivo parceiro contratual do site . Antes da celebração do contrato, o utilizador é chamado a atenção para isso de forma adequada.",
                
                "Os regulamentos que divergem destas condições ou as condições gerais de negócio do utilizador só se aplicam se o Admin do jogo o autorizar previamente por escrito.",
        ...
        "Celebração do contrato"                      => array( 
                "O pressuposto para a utilização dos jogos online e outras ofertas de serviços disponibilizados por este site é o registo do utilizador.",
                
                "O registo só é permitido a pessoas físicas. Só são permitidas pessoas individuais como utilizadores (são proibidos grupos, famílias, parceiros, etc.). Caso o utilizador seja um menor, este assegura com o envio do requerimento de registo que existe uma autorização legal dos seus representantes legais.",
                
                "Ao efectuar o registo, o utilizador deve indicar um nome de jogador e um endereço de e-mail registado em seu nome. O utilizador não tem direito à atribuição de um determinado nome de jogador. O nome do jogador não deve violar os direitos de terceiros e também não deve infringir as boas maneiras. Para além disso, também não é permitido escolher um endereço de e-mail ou de Internet como nome de jogador. O utilizador responsabiliza-se pelo facto de as informações prestadas por ele no registo do jogo serem verdadeiras e completas, oo aceitar o registo aceita também o envio de publicidade destinada a noticifar os Utilizadores de eventos  relacionados com este site, e os seus dados guardados são de excluziva pertence do webMaster que se compromete em não os divulgar para outros fins que não sejam do restrito relacionamento e interesse deste site",
                ...
                "Com o registo bem sucedido, o utilizador cria uma conta para jogar e o utilizador poderá administrar por ele próprio. O utilizador pode criar um conta tanto no site de portal do jogo como também no respectivo site do jogo online e fórum da comunidade. A conta criado no portal do jogo também pode ser utilizado pelo utilizador para todos os outros jogos online ligados ao jogo por link ao portal no jogo. De modo inverso, A Conta criada numa página de jogos online não é utilizável para outros jogos online.",
                
                "A Conta não é transmissível sem a autorização expressa da equipa deste jogo.",
        ...
        "Utilização das páginas de Internet deste site e conteúdos das páginas de Internet"               => array(
                "As páginas de Internet deste site têm conteúdos de todo o tipo que estão protegidos pela lei relativa a marcas, direitos de autor e de outra forma a favor do Amin do jogo ou a favor de terceiros. Desde que não esteja expressamente permitido no âmbito destas condições de utilização, o utilizador não está autorizado a processar, fotocopiar, divulgar, reproduzir publicamente, publicitar ou utilizar, para além do que foi acordado contratualmente, as páginas de Internet deste site ou os conteúdos ou partes individuais disponibilizados. Só é permitida a reprodução condicionada pela técnica para efeitos de browsing, assim como a reprodução permanente apenas para uso privado. As indicações de direitos de autor e as designações de marcas não podem ser alteradas nem suprimidas ou eliminadas.",
                ...
                "O utilizador responsabiliza-se a abster-se de todas as medidas que afectam ou perturbam o modo de funcionamento das páginas de Internet deste site ou serviços e prestações de serviço aqui oferecidos, assim como a não aceder a dados para os quais o utilizador não tem autorização. A consulta dos conteúdos disponibilizados só deve realizar-se de modo a não afectar a utilização das páginas de Internet deste site e dos conteúdos pelos restantes utilizadores. Não é permitido o envio de dados ou software que podem influenciar o hardware ou o software de receptores.",
                
                "Qualquer utilização das páginas de Internet deste site para efeitos comerciais, nomeadamente publicitários requer a expressa autorização prévia, por escrito, do Admin do Jogo.",
                
                "O utilizador não tem qualquer direito à publicação de conteúdos nas páginas de Internet do jogo.",
                
                "Não é permitida a utilização das páginas de Internet deste site através de serviços de anonimização que suprimem o verdadeiro endereço IP do utilizador.",
                
                "É permitido colocar um link para as páginas de Internet deste site desde que este link sirva apenas para a referência cruzada. No entanto, o Admin deste jogo reserva-se o direito de revogar esta autorização. No entanto, não é permitido integrar ou apresentar as páginas de Internet deste site ou os respectivos conteúdos através de uma hiperligação numa janela parcial (Frame).",
        ...
        "Condições especiais para a utilização dos jogos online"               => array(
                "Em cada servidor de um jogo online (p. ex, Mundo, Universo, etc.), o utilizador só pode participar com uma conta, a não ser que as regras do jogo prevejam excepções. Não é permitida a utilização de várias contas. Estas “multi contas” podem ser eliminadas e bloqueadas livremente pela equipa do jogo a qualquer momento.",
                
                "Não é permitido ao utilizador intervir de forma manipulativa no jogo online. Nomeadamente, o utilizador não está autorizado a utilizar medidas, mecanismos ou software que possam perturbar o funcionamento e o normal decorrer do jogo. O utilizador não deve tomar medidas que possam ter como consequência a sobrecarga não razoável ou em demasia das capacidades técnicas. Não é permitido ao utilizador bloquear conteúdos gerados pela gestão do jogo, sobrescrevê-los ou modificá-los ou intervir de outra forma qualquer no jogo, perturbando-o.",
                
                "Além disso, não é permitido ao utilizador aceder ao jogo (incluindo todas as páginas de Internet individuais) com outros programas, excepto com o Internet Browser ou o programa Client disponibilizado. Isto refere-se nomeadamente aos chamados Bots, assim como a outras ferramentas que devem substituir ou complementar a interface web. Também não são permitidos scripts e programas completa ou parcialmente automatizados que dão ao utilizador uma vantagem em relação aos outros jogadores. Aqui também estão incluídas funções de “auto refresh” e outros mecanismos integrados do browser de Internet, desde que se tratem de processos automatizados.",
                
                "Em nenhumas circunstâncias o usuário pode
	         a) Elaborar ou utilizar cheats, mods e/ou hacks, assim como utilizar qualquer outro software elaborado por terceiros que altere a experiência deste Jogo online,
	         b) Utilizar software que permita o Datamining ou que, de outra forma, capte e recolha de outro modo informações relacionadas com  este jogo online,
	         ...
	         Isto também abrange todos os contornos, modos de actuação semelhantes ou que, quanto ao seu efeito, são iguais às proibições acima mencionadas.",
                ...
                "Só é permitido efectuar o Login no jogo através da página inicial do respectivo site e através da página de Home do jogo. Não é permitida a abertura automatizada da conta, independentemente do facto de a página inicial ser apresentada ou não.",
                
                "Todos os direitos relativamente aos objectos virtuais utilizados nos jogos online e deixados a título oneroso são exclusivos do Admin deste jogo ou dos parceiros contratuais autorizados pelo Administrador. O mesmo é válido para os objectos virtuais criados pelo próprio utilizador. O utilizador concede ao Administrador um direito de utilização exclusivo, limitado no espaço, no tempo e nos conteúdos, destes objectos virtuais por ele criados. Este direito inclui nomeadamente os direitos de reprodução, divulgação e processamento. O utilizador recebe apenas nos objectos virtuais um direito de utilização não exclusivo e limitado ao prazo do contrato.",
        ),
        ...
        "Consequência da violação de deveres" => array(
	         ...
	         d) Bloqueio provisório ou permanente de um utilizador para jogos online individuais ou para todos os jogos online e conteúdos das páginas de Internet deste site de jogos online,
        ...
        "Recompensa relativamente à utilização"                               => array(
                ...
                "No entanto, o utilizador pode ajudar na manutenção este site, o qual não tem fins lucrativos nem pretende ser uma empresa, as promuções enviadas são exclusivamente para a ajuda na manutenção do servidor bem como do dominio, Não existe vendas, só contribuiçõs feitas de livre vontade pelos utilizadores",
                
                "Se um utilizador menor desejar contribuir este site,  deve  ser feita pelo representante legal para este efeito ou para utilização livre.",
                
                "O utilizador assegura que todas as informações prestadas no âmbito de um processo de contrinuição (nomeadamente dados bancários, número de cartão de crédito, etc.) estão completas e correctas.",
        ...
        "Limitação de responsabilidade"      => array(
                ...
                "Caso algum Moderador da equipa ou mesmo a equipa preste o respectivo complemento que activa a responsabilidade a título superior, a equipa responsabiliza-se apenas pela intenção dolosa e pela negligência grave.",
                
                "Nos complementos a título superiror, o Admin responsabiliza-se ilimitadamente pela intenção dolosa e pela negligência grave, assim como no caso de danos em pessoas, no entanto quanto à negligência leve apenas se forem violados deveres essenciais ao contrato, em caso de atraso por parte do Admin e impossibilidade. A responsabilidade em caso de violação de um destes deveres essenciais ao contrato está limitada ao dano típico contratual com cujo surgimento o Admin teve de contar na celebração do contrato devido às circunstâncias conhecidas nessa altura.",
                
                "A equipa não assume responsabilidade por falhas não cometidas pelo usuário dentro das redes.",
                
                "A equipa só se responsabiliza pela perda de dados de acordo com os parágrafos mencionados previamente se não fosse possível evitar esta perda através de medidas adequadas de protecção de dados por parte do utilizador.",
                
                "As limitações de responsabilidade existentes não são válidas no caso de aceitação de garantias expressas por este jogo, no caso de malícia e no caso de danos resultantes da colocação em perigo de vida, do corpo ou saúde, assim como no caso de regulamentos legais obrigatórios.",
         ),


         
         "Privacy"      => "Os dados pessoais do utilizador só são recolhidos, processados ou utilizados para noticias informação e promuções com fins não lucrativos, estes email enviados nunca poderam ser considerados SPAM já que a aceitação do registo define que o sistema do jogo possa enviar tais informações usando para isso o sistema de mensagens integrado no jogo",
         );
