...
{block name="content"}
	...
	<tr>
		<td>
			<a href="?page=fleetTable&amp;galaxy={$galaxy}&amp;system={$system}&amp;planet={$planet}&amp;planettype=1">{$planet}</a>
		</td>
    ...
    {else}
		...
		<td style="white-space: nowrap;">
			...
			<a class="tooltip_sticky" name="<table style='width:240px'><tr><th>{$LNG.gl_alliance} {$currentPlanet.alliance.name} {$currentPlanet.alliance.member}</th></tr><td><table><tr><td><a href='?page=alliance&amp;mode=info&amp;id={$currentPlanet.alliance.id}'>{$LNG.gl_alliance_page}</a></td></tr><tr><td><a href='?page=statistics&amp;start={$currentPlanet.alliance.rank}&amp;who=2'>{$LNG.gl_see_on_stats}</a></td></tr>{if $currentPlanet.alliance.web}<tr><td><a href='{$currentPlanet.ally.web}' target='allyweb'>{$LNG.gl_alliance_web_page}</td></tr>{/if}</table></td></table>">
				<span class="galaxy-alliance{if $currentPlanet.alliance.inally} galaxy-alliance-member{/if}">{$currentPlanet.alliance.tag}</span>
			</a>
			{else}-{/if}
		</td>
		<td style="white-space: nowrap;">
	...
	<tr>
		<td colspan="3">
			<span id="probes">{$spyprobes|number}</span> {$LNG.gl_avaible_spyprobes}
		</td>
		<td colspan="3">
			<span id="recyclers">{$recyclers|number}</span> {$LNG.gl_avaible_recyclers}
		</td>
		<td colspan="2">
			<span id="grecyclers">{$grecyclers|number}</span> {$LNG.gl_avaible_grecyclers}
		</td>
	</tr>
