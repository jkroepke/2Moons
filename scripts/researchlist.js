function ReBuildView() {
	...
	$.each(data.build, function(index, val) {
		...
		if (index == 0) {
			...
			HTML	+= '<td><div id="time"></div><div id="command"><a href="game.php?page=buildings&mode=research&amp;cmd=cancel" class="post">'+data.bd_cancel+'</a></div>';
		} else {
			HTML	+= '<td style="width:70%" class="left">'+(index+1)+'.: '+val.name+' '+val.level+' '+(val.planet != '' ? ' @ '+val.planet : '')+'</td>';
			HTML	+= '<td><a href="game.php?page=buildings&mode=research&amp;cmd=remove&amp;listid='+(index+1)+'" class="post">'+data.bd_cancel+'</a>';
		}
