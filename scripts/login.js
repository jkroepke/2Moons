...
var Dialog	= {
	...
	create: function(button, div) {
		...
		if($(div).length === 0) {
			...
			$(div).css('max-height', $(window).height() * 0.8 - 50).dialog({
				autoOpen: false,
				modal: true,
				width: 650,
	...
	standart: function(title, mode, uni){
		Dialog.create();
		$(Dialog.div).dialog('option', 'title', title).dialog('open').load('?page='+mode+(typeof uni === "number" ? '&uni='+uni : ''));
		return false;
	},
