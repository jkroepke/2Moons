				...
				<tbody>
					<tr>
						<td class="transparent left" style="width:90%;padding:10px;">
							...
							{foreach $Element.elementBonus as $BonusName => $Bonus}{if $Bonus@iteration % 3 === 1}<p>{/if}{if $Bonus < 0}{$VZ = "-{abs($Bonus * 100)}"}{else}{$VZ = "+{abs($Bonus * 100)}"}{/if}{$LNG.bonus.$BonusName|replace:'%f%':$VZ}{if $Bonus@iteration % 3 === 0 || $Bonus@last}</p>{else}&nbsp;{/if}{/foreach}
				...
				<tbody>
					<tr>
						<td class="transparent left" style="width:90%;padding:0px 10px 10px 10px;">
							<p>{$LNG.shortDescription.{$ID}}</p>
							<p>{foreach $Element.elementBonus as $BonusName => $Bonus}{if $Bonus < 0}{$VZ = "-{abs($Bonus * 100)}"}{else}{$VZ = "+{abs($Bonus * 100)}"}{/if}{$LNG.bonus.$BonusName|replace:'%f%':$VZ}<br>{/foreach}</p>
