...
<tr>
	...
	<td><a href="javascript:playercard('game.php?page=playercard&amp;id={$RangeInfo.id}')"{if $RangeInfo.id == $CUser_id} style="color:lime"{/if}>{$RangeInfo.name}</a></td>
	<td><a href="javascript:f('game.php?page=messages&amp;mode=write&amp;id={$RangeInfo.id}','');"><img src="{$dpath}img/m.gif" title="{$st_write_message}" alt="{$st_write_message}"></a></td>
