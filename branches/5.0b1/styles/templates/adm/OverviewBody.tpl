<script>document.body.style.overflow = "auto";</script>
<style>table.lic{background-color:#050E1F;border:2px {color} solid;}
th.lic{border:0px;}</style>
<body>
<h1>{ow_title}</h1>
<table width="90%" class="lic">
<tr>
    <th class="lic">{error_message}</th>
</tr>
</table>

<br>
<table width="80%">
	<tr>
    	<td class="c">{ow_overview}</td>
    </tr>
	<tr>
    	<th height="50px"><div align="center">{ow_welcome_text}</div></th>
    </tr>
    <tr>
        <td class="c">{ow_support}</td>
    </tr>
    <tr>
        <th><a href="http://www.xnova-reloaded.de/" target="_blank">xnova-reloaded.de - {ow_forum}</a><br>
        <a href="http://www.xtreme-gamez.com.ar/foros/" target="_blank">xtreme-gamez.com.ar - {ow_forum}</a></th>
    </tr>   
    <tr>
    	<td class="c">{ow_credits}</td>
    </tr>
    <tr>
    	<th align="center">
            <table width="475">
                <tr>
                  <th>
					<h3>{ow_proyect_leader}</h3>
					<h3><font color="red">Slaver</font></h3>
				  </th>
                </tr>
		        <tr>
				  <th>
					<h3>{ow_translator}</h3>
					Slaver<br>Brandi (ACP)<br><br>
				  </th>
                </tr> 
                <tr>
                  <th>
					<h3>{ow_special_thanks}</h3>
					Inforcer<br>Mori<br>Metusalem<br>Meikel<br>Phil<br>Schnippi<br>Vobi<br>Onko<br>Sycrog<br>Allen Spielern im <a href="http://www.titanspace.de" alt="Betauni">Betauni</a><br>
				  </th>
                </tr>				
            </table>
        </th>
    </tr>
</table>
</body>