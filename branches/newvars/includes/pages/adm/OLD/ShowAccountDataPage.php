...
function ShowAccountDataPage()
{
	...
	if (!empty($id_u))
	{
		...
		else
		{
			...
			{
				$SpecifyItemsUQ	.= "u.`".$GLOBALS['VARS']['ELEMENT'][$ID]['name']."`,";
			}
			...
			{
				$techoffi .= isset($techno[$i]) ? "<tr><td>".$LNG['tech'][$techno[$i]].": <font color=aqua>".$UserQuery[$GLOBALS['VARS']['ELEMENT'][$techno[$i]['name']]]."</font></td>" : "<tr><td>&nbsp;</td>";
				
				$techoffi .= isset($officier[$i]) ? "<td>".$LNG['tech'][$officier[$i]].": <font color=aqua>".$UserQuery[$GLOBALS['VARS']['ELEMENT'][$officier[$i]['name']]]."</font></td></tr>" : "<td>&nbsp;</td></tr>";				
			}
			...
			foreach(array_merge($reslist['fleet'], $reslist['build'], $reslist['defense']) as $ID)
			{
				...
				$RES[$GLOBALS['VARS']['ELEMENT'][$ID]['name']]	= "<tr><td width=\"150\">".$LNG['tech'][$ID]."</td>";
			}
			...
			while ($PlanetsWhile	= $GLOBALS['DATABASE']->fetchArray($PlanetsQuery))
			{
				...
				{	
					...
					foreach(array_merge($reslist['fleet'], $reslist['build'], $reslist['defense']) as $ID)
					{
						$RES[$GLOBALS['VARS']['ELEMENT'][$ID]['name']]	.= "<td width=\"60\"><a title=\"".pretty_number($PlanetsWhile[$GLOBALS['VARS']['ELEMENT'][$ID]['name']])."\">".shortly_number($PlanetsWhile[$GLOBALS['VARS']['ELEMENT'][$ID]['name']])."</a></td>";
					}
			...
			foreach(array_merge($reslist['fleet'], $reslist['build'], $reslist['defense']) as $ID)
			{
				$RES[$GLOBALS['VARS']['ELEMENT'][$ID]['name']]	.= "</tr>";
			}
			
			foreach($reslist['build'] as $ID)
			{
				$build	.= $RES[$GLOBALS['VARS']['ELEMENT'][$ID]['name']];
			}
			
			foreach($reslist['fleet'] as $ID)
			{
				$fleet	.= $RES[$GLOBALS['VARS']['ELEMENT'][$ID]['name']];
			}
