...
function ShowFlyingFleetPage()
{
	...
	while($fleetRow = $GLOBALS['DATABASE']->fetchArray($fleetResult)) {
		...
		$FleetList[]	= array(
			...
			'starttime'				=> DateUtil::formatDate($LNG['php_tdformat'], $fleetRow['start_time'], $USER['timezone']),
			'arrivaltime'			=> DateUtil::formatDate($LNG['php_tdformat'], $fleetRow['fleet_start_time'], $USER['timezone']),
			'stayhour'				=> round(($fleetRow['fleet_end_stay'] - $fleetRow['fleet_start_time']) / 3600),
			'staytime'				=> $fleetRow['fleet_start_time'] !== $fleetRow['fleet_end_stay'] ? DateUtil::formatDate($LNG['php_tdformat'], $fleetRow['fleet_end_stay'], $USER['timezone']) : 0,
			'endtime'				=> DateUtil::formatDate($LNG['php_tdformat'], $fleetRow['fleet_end_time'], $USER['timezone']),
