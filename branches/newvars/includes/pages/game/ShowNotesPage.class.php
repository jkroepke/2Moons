...
class ShowNotesPage extends AbstractPage
{
	...
	public function insert()
	{
		...
		if($id == 0) {
			$SQL	= "INSERT INTO ".NOTES." SET owner = ".$USER['id'].", time = ".TIMESTAMP.", priority = ".$priority.", title = '".$GLOBALS['DATABASE']->escape($title)."', text = '".$GLOBALS['DATABASE']->escape($text)."', universe = ".$UNI.";";		
		} else {
			$SQL	= "UPDATE ".NOTES." SET time = ".TIMESTAMP.", priority = ".$priority.", title = '".$GLOBALS['DATABASE']->escape($title)."', text = '".$GLOBALS['DATABASE']->escape($text)."' WHERE id = ".$id.";";
		}
