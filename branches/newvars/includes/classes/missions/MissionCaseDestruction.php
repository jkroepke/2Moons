...
class MissionCaseDestruction extends MissionFunctions
{
	...
	function TargetEvent()
	{
		...
		for ($i = 200; $i < 500; $i++)
		{
			...
			{
				$defense[0]['def'][$i] = $targetPlanet[$GLOBALS['VARS']['ELEMENT'][$i]['name']];
			}
		}
		...
		$EndPlanet		= $GLOBALS['DATABASE']->getFirstRow("SELECT `id` FROM ".PLANETS." WHERE `id_luna` = '".$this->_fleet['fleet_end_id']."';");
