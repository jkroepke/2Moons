<div class="spyRaport">
	...
	{if $elementID != 921 && (($Class == 100 && $targetUser[$resource[$elementID]] > $condition) || ($Class != 100 && $targetPlanet[$resource[$elementID]] > $condition))}
	...
	<div class="spyRaportFooter">
		...
		{if $isBattleSim}<br><a href="game.php?page=battleSimulator{foreach $classIDs as $Class => $elementIDs}{foreach $elementIDs as $elementID}{if $elementID != 921}&amp;im[{$elementID}]={if $Class == 100}{round($targetUser[$resource[$elementID]])}{else}{round($targetPlanet[$resource[$elementID]])}{/if}{/if}{/foreach}{/foreach}">{$LNG.fl_simulate}</a>{/if}
	</div>
</div>